/**
 * Thermo-Nuclear Code Quality Review Extension
 *
 * Provides a `/thermo-nuclear-review` command that prompts the agent to review code changes.
 ...
 *
 * Usage:
 * - `/thermo-nuclear-review` - show interactive selector
 * - `/thermo-nuclear-review pr 123` - review PR #123 (checks out locally)
 * - `/thermo-nuclear-review pr https://github.com/owner/repo/pull/123` - review PR from URL
 * - `/thermo-nuclear-review uncommitted` - review uncommitted changes directly
 * - `/thermo-nuclear-review branch main` - review against main branch
 * - `/thermo-nuclear-review commit abc123` - review specific commit
 * - `/thermo-nuclear-review folder src docs` - review specific folders/files (snapshot, not diff)
 * - `/thermo-nuclear-review` selector includes Add/Remove custom review instructions (applies to all modes)
 * - `/thermo-nuclear-review --extra "focus on performance regressions"` - add extra review instruction (works with any mode)
 *
...
// This is intentional - the UI and /thermo-nuclear-end-review command assume a single active review.
let reviewOriginId: string | undefined = undefined;
let endReviewInProgress = false;
let reviewCustomInstructions: string | undefined = undefined;

const REVIEW_STATE_TYPE = "thermo-nuclear-review-session";
const REVIEW_ANCHOR_TYPE = "thermo-nuclear-review-anchor";
const REVIEW_SETTINGS_TYPE = "thermo-nuclear-review-settings";
...
function setReviewWidget(ctx: ExtensionContext, active: boolean) {
	if (!ctx.hasUI) return;
	if (!active) {
		ctx.ui.setWidget("thermo-nuclear-review", undefined);
		return;
	}

	ctx.ui.setWidget("thermo-nuclear-review", (_tui, theme) => {
		const message = "Review session active, return with /thermo-nuclear-end-review";
...
async function loadThermoNuclearReviewSkill(): Promise<string> {
	return await fs.readFile(new URL("./THERMO_NUCLEAR_CODE_QUALITY_REVIEW.md", import.meta.url), "utf8");
}
...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	async function executeReview(
		...
		if (reviewOriginId) {
			ctx.ui.notify("Already in a review. Use /thermo-nuclear-end-review to finish first.", "warning");
			return false;
		}
		...
		// Combine the thermo-nuclear review skill with the specific review target.
		let fullPrompt = `${await loadThermoNuclearReviewSkill()}\n\n---\n\nPlease perform a code review with the following focus:\n\n${prompt}`;
	...
	// Register the /thermo-nuclear-review command
	pi.registerCommand("thermo-nuclear-review", {
		...
		handler: async (args, ctx) => {
			...
			while (true) {
				...
				// In an empty session, default to fresh review mode so /thermo-nuclear-end-review works consistently.
...
9. End with: fixed items, deferred/skipped items (with reasons), and verification results.`;
	...
	async function executeEndReviewAction(
		...
		if (!originId) {
			if (!getReviewState(ctx)?.active) {
				ctx.ui.notify("Not in a review branch (use /thermo-nuclear-review first, or review was started in current session mode)", "info");
			}
			return "error";
		}
		...
		if (action === "returnOnly") {
			try {
				...
				if (result.cancelled) {
					ctx.ui.notify("Navigation cancelled. Use /thermo-nuclear-end-review to try again.", "info");
					return "cancelled";
				}
			} catch (error) {
		...
		if (summaryResult === null) {
			ctx.ui.notify("Summarization cancelled. Use /thermo-nuclear-end-review to try again.", "info");
			return "cancelled";
		}
	...
	async function runEndReview(ctx: ExtensionCommandContext): Promise<void> {
		...
		if (endReviewInProgress) {
			ctx.ui.notify("/thermo-nuclear-end-review is already running", "info");
			return;
		}
		...
		try {
			...
			if (choice === undefined) {
				ctx.ui.notify("Cancelled. Use /thermo-nuclear-end-review to try again.", "info");
				return;
			}
	...
	// Register the /thermo-nuclear-end-review command
	pi.registerCommand("thermo-nuclear-end-review", {
